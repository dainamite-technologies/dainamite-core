...
import type { SeedScope } from './seeders'

// GIX-specific wizard definition. Migrated from src/modules/cpq/lib/seeds.ts
// (XD-210). Generic wizards still ship with the cpq module — only the
// GIX-branded wizard moves here so cpq remains catalog-agnostic.
...
const GIX_WIZARDS = [
  {
    ...
    description:
      'Guided 4-step flow: select customer, configure access port, choose additional services, review summary.',
    surface: 'backend',
    ...
    steps: [
      ...
      },
      {
        ...
        config: {
          offeringFilter: {
            offeringCodes: ['OFFER-GIX-INTERNET-PEERING', 'OFFER-GIX-MOBILE-PEERING', 'OFFER-GIX-CLOUD-CONNECT'],
          },
...
export async function seedGixWizards(em: EntityManager, scope: SeedScope): Promise<void> {
  ...
  console.log('    GIX wizard seed complete.')
}
