    ...
    setup(build) {
      build.onEnd(async (result) => {
        if (result.errors.length > 0) {
          console.error(`[connector:watch] Build failed with ${result.errors.length} error(s)`)
          return
        }
        ...
        console.log(`[connector:watch] Rebuilt ${outputFiles.length} files at ${new Date().toLocaleTimeString()}`)
      })
    },
  }
}
...
async function createContext() {
  ...
  if (entryPoints.length === 0) {
    console.error('[connector:watch] No source entry points found!')
    process.exit(1)
  }
...
async function rebootContext(reason) {
  if (currentContext) {
    try {
      ...
      console.error('[connector:watch] Failed to dispose previous context:', err?.message ?? err)
    }
  }
  console.log(`[connector:watch] ${reason} — restarting esbuild context`)
  const { ctx, entryPointCount } = await createContext()
  currentContext = ctx
  await ctx.rebuild()
  await ctx.watch()
  console.log(`[connector:watch] Tracking ${entryPointCount} entry points`)
}

console.log(`[connector:watch] Starting watcher`)
const initial = await createContext()
currentContext = initial.ctx
console.log(`[connector:watch] Found ${initial.entryPointCount} source entry points`)
...
console.log('[connector:watch] READY')

await initial.ctx.watch()
console.log('[connector:watch] Watching for changes — Ctrl+C to stop')
