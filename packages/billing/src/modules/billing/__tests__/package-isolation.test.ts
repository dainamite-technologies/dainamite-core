/**
 * Package isolation guard for @dainamite/billing.
 *
 * Mirrors the @dainamite/cpq package-isolation test — proves the billing
 * package can be loaded without any sibling `src/modules/<x>` modules and
 * that every import resolves to a peerDependency or sibling file. See the
 * cpq variant for the full rationale; this version reflects billing's
 * narrower allowed-imports and required-modules surface.
 */
...
// Must mirror peerDependencies in packages/billing/package.json (plus
// relatives + node builtins). Adding a prefix here without adding it to
// peerDependencies = silent runtime failure for consumers that don't have
...
describe('@dainamite/billing package isolation', () => {
  let allFiles: string[] = []
  ...
  it('declares all required modules in metadata.requires', () => {
    expect(metadata.requires).toEqual(
      expect.arrayContaining(['auth', 'directory', 'dictionaries', 'configs', 'sales']),
    )
  })
  ...
  it('contains no @ManyToOne / @OneToMany / @ManyToMany / @OneToOne ORM relations', async () => {
    ...
    if (violations.length > 0) {
      throw new Error(
        `Cross-entity ORM relations forbidden in @dainamite/billing (use FK string columns):\n${violations
  ...
  it('module index.ts loads without throwing', async () => {
    ...
    expect(mod.metadata.name).toBe('billing')
